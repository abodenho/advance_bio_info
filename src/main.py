...
def get_experiment_values(test_number):
    ...
    ESPILON_MIN = None
    NW_MODE = 0
    if test_number == test_class.EPSILON_DECAY_Q.value:
        EPSILON = 1
        ESPILON_DECAY = 0.95
        ESPILON_MIN = 0.01

    elif test_number == test_class.WITHOUT_TRUNCATURE_Q.value:
        TRONCATE = False

    elif test_number == test_class.EXTEND_GAP_MSA.value:
        NW_MODE = 1

    elif test_number == test_class.DIFFENT_VALUE_MSA.value:
        NW_MODE = 2

    elif test_number == test_class.PRIORITY_ORDER_MSA.value:
        NW_MODE = 3
